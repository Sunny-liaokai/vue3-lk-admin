...
import { uniqueSlash } from "@/util/urlUtil";
  ...
  return routes
    ...
    .map((item) => {
      ...
      if (item.type === 0) {
        ...
        const Component = loadComponents(viewPath) || NotFound;
...
export const loadComponents = (viewPath) => {
...
export const generatorDynamicRouter = (asyncMenus: API.Menu[]) => {
  try {
    ...
    // 给公共路由添加namePath
    generatorNamePath(baseRouters);
    const menus = [...baseRouters, ...routeList, ...endRoutes];
